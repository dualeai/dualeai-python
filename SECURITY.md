...
- Cross-Tenant cache access or deletion caused by SDK-owned key construction or cache
  clearing.
...
- **Redis cache clearing is Tenant-scoped.** Keys use the
  `dualeai:sdk:{tenant_id}:` namespace and `clear()` scans and deletes only that
  prefix. `tests/test_cache.py::TestRedisCacheCommands` holds the Redis command
  boundary. The SDK does not recognize or migrate older key formats.
- **Redis credentials stay out of initialization logs.** Both `redis://` and
  ...
  the initialization event. Held by
  `tests/test_cache.py::TestRedisCacheCommands::test_tls_url_reaches_redis_without_reaching_logs`
  and `tests/test_config.py::TestConfigValidation::test_redis_url_accepts_plain_and_tls_schemes`.
...
pypi-attestations verify pypi \
  ...
  https://files.pythonhosted.org/packages/.../dualeai-<version>-py3-none-any.whl
```
